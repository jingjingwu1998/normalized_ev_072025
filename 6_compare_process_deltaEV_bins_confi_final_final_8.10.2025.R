## ================================================================
## EV Δ comparisons + export SAME/DIFF bins/regions (RDS + BED)
## ================================================================
setwd("/Users/80030577/Desktop/HiC_analysis/EV_normalization")
load("normalized_ev.100k_multi_samples_06_03_2025.rda")
setwd("/Users/80030577/Desktop/HiC_analysis/EV_normalization/test")
...
## Merge consecutive bins by sign, per chromosome (no cross‑chr merges)
group_consecutive_sign <- function(gr, sign_col = "sign") {
  if (length(gr) == 0L) return(gr)
  split_by_chr <- split(gr, seqnames(gr))
  merged_list <- lapply(split_by_chr, function(g) {
    signs <- mcols(g)[[sign_col]]
    r     <- Rle(signs)
    gid   <- inverse.rle(list(values = seq_along(runLength(r)),
                              lengths = runLength(r)))
    g$group <- gid
    out <- reduce(split(g, g$group))
    out <- unlist(out)
    ## carry sign for each merged segment
    out$sign <- signs[match(names(out), names(g))]
    out
  })
  unlist(GRangesList(merged_list))
}

## ---------------------------------------------
## Step 4: Pairwise comparison helper
## ---------------------------------------------
...
## (Optional) CSV of DIFF bins (as before)
write.csv(as.data.frame(result_rbl_gcbc_vs_nbc_gcbc$diff), "diff_rbl_gcbc_vs_nbc_gcbc.csv", row.names = FALSE)
write.csv(as.data.frame(result_rbl_lcl_vs_nbc_lcl$diff),   "diff_rbl_lcl_vs_nbc_lcl.csv",   row.names = FALSE)
write.csv(as.data.frame(result_rbl_lcl_vs_rbl_nbc$diff),   "diff_rbl_lcl_vs_rbl_nbc.csv",   row.names = FALSE)

## ================================================================
## New: Export SAME/DIFF bins and merged regions (RDS + BED)
## ================================================================
export_gr_rds <- function(gr, path_rds) {
...
export_gr_bed <- function(gr, path_bed, name_prefix = "feat") {
  ...
  df <- data.frame(
    ...
    end      = GenomicRanges::end(gr),         # keep GRanges end (1-based) → fine for BED end
  ...
  ## Append sign/deltaEV into name for quick inspection (if present)
...
cat("✅ Exported SAME/DIFF bins and merged regions as .rds (GRanges) and .bed (0-based).\n")
